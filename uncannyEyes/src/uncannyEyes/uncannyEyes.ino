...
#ifdef _ADAFRUIT_ST7735H_ // TFT
  if (e==0) {  eye[e].display.setRotation(0);}
  else {eye[e].display.setRotation(0);}
  //eye[e].display.initR(INITR_18BLACKTAB);
  //eye[e].display.setAddrWindow(16, 0, 143, 128);
  if ((Select_Heart == 0) && (Count == 0))
  {eye[e].display.fillScreen(ST7735_BLACK); Count=1;}
  
  else if ((Select_Heart == 1) && (Count == 0))
  {eye[e].display.fillScreen(ST7735_WHITE); 
  Count=1;}
  
  eye[e].display.setAddrWindow(0, 10, 127, 149);
#else // OLED
 ...
 // Serial.write("OKOK");
  ...
  int32_t d;
    ...
    else if (inChar == 'h')
    {
      ...
      if (Select_Heart ==0)  
      {Select_Heart = 1;
        eye[0].display.fillScreen(ST7735_WHITE);
        eye[1].display.fillScreen(ST7735_WHITE);
      }
